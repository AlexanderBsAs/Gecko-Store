  ...
  usersInDb: async (req, res) => {
    try {
      ...
      if (users.length > 0) {
        ...
        userJson.detail = `http://localhost:3000/api/users/${user.id}`;
        return userJson;
        });
  ...
  userDetail: async (req,res)=>{
    try {
      ...
      if(!Number.isInteger(id)){
        ...
        const user = await db.User.findByPk(id,{ include: ["addresses"]});
        if(!user){
          throw new Error(`El usuario con el ID ${id} no existe en la base de datos.`);
        } else {
          const userDetails = user.toJSON();
          delete userDetails.password;
          delete userDetails.rol_id;
